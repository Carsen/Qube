...
// RandomKey returns a random key of length n
...
// RandomValue returns a random key of length n
func RandomValue(n int) Value {
	bytes := make([]byte, n)
	rand.Read(bytes)
	for i, b := range bytes {
		bytes[i] = alphabet[b%byte(len(alphabet))]
...
func TestLocking(t *testing.T) {
	...
	rdb, err := Open(testDir, WithOpenReadonly(true))
	require.NoError(t, err)
...
func TestTransactions(t *testing.T) {
	...
	// Ensure datafile is rotated
	for i := 0; i <= 100; i++ {
		require.NoError(t, db.Put(RandomKey(15), RandomValue(32)))
	}

	val, err := txn.Get(Key("foo"))
	require.NoError(t, err)
...
func TestReader(t *testing.T) {
	...
	db, err := Open(testDir, WithMaxValueSize(1<<24))
	assert.NoError(t, err)
	defer db.Close()

	require.NoError(t, db.Put(Key("foo"), RandomValue(1<<20)))

	r, err := db.GetReader(Key("foo"))
	require.NoError(t, err)
	_, err = io.ReadAll(r)
	require.NoError(t, err)
}
